...
const people = [
    ...
    //  2. Array of the inventory first and last names
    const inventorsNames = inventors.map( (name) => {
        return `${name.first} ${name.last}`
    } )
    //console.table(inventorsNames)

    //  3. Sort the inventors by birth date, oldest toy youngest
  ...
  const totalYears = inventors.reduce((total, inventor) => {
    ...
    //  5. Sort inventors by years they lived
